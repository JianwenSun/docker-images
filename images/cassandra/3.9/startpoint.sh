...
if [[ -z "$CASSANDRA_HOST_IP" ]]; then
    export CASSANDRA_HOST_IP=$CASSANDRA_IP
fi
...
if [[ -z "$CASSANDRA_ENABLE_USER_DEFINED_FUNCTIONS" ]]; then
    export CASSANDRA_ENABLE_USER_DEFINED_FUNCTIONS="true"
fi

if [[ -z "$CASSANDRA_ENABLE_SCRIPTED_USER_DEFINED_FUNCTIONS" ]]; then
    export CASSANDRA_ENABLE_SCRIPTED_USER_DEFINED_FUNCTIONS="true"
fi
...
for VAR in `env`
do
  if [[ $VAR =~ ^CASSANDRA_ && ! $VAR =~ ^CASSANDRA_HOME && ! $VAR =~ ^CASSANDRA_USER && ! $VAR =~ ^CASSANDRA_HOST_IP &&  ! $VAR =~ ^CASSANDRA_VERSION && ! $VAR =~ ^CASSANDRA_SEEDS ]]; then
